...
struct ContentView: View {
    ...
    @State private var selectedMode: GuideMode = .koten // 🌟 テスト用に初期値を「交点」に設定
    ...
    class Coordinator: NSObject, ARSessionDelegate {
                ...
                case .henTsukuri:
                    ...
                    let drawGuides = { (baseX: Float) in
                        if case let .henTsukuri(splitX) = guideType {
                            textContainer.addChild(createLineEntity(from: SIMD3<Float>(baseX + splitX, currentY + fixedBoxSize / 2, 0), to: SIMD3<Float>(baseX + splitX, currentY - fixedBoxSize / 2, 0), thickness: lineThickness * 1.5, color: UIColor.blue.withAlphaComponent(0.4)))
                        } else if case let .center(centerX) = guideType {
                            textContainer.addChild(createLineEntity(from: SIMD3<Float>(baseX + centerX, currentY + fixedBoxSize / 2, 0), to: SIMD3<Float>(baseX + centerX, currentY - fixedBoxSize / 2, 0), thickness: lineThickness * 1.5, color: UIColor.green.withAlphaComponent(0.4)))
                        } else if case let .shinnyo(splitX, bottomY) = guideType {
                            textContainer.addChild(createLineEntity(from: SIMD3<Float>(baseX + splitX, currentY + fixedBoxSize / 2, 0), to: SIMD3<Float>(baseX + splitX, currentY + bottomY, 0), thickness: lineThickness * 1.5, color: UIColor.orange.withAlphaComponent(0.4)))
                            textContainer.addChild(createLineEntity(from: SIMD3<Float>(baseX + splitX, currentY + bottomY, 0), to: SIMD3<Float>(baseX + fixedBoxSize / 2, currentY + bottomY, 0), thickness: lineThickness * 1.5, color: UIColor.orange.withAlphaComponent(0.4)))
                        } else if case let .kamae(leftX, rightX, topY, bottomY) = guideType {
                            textContainer.addChild(createLineEntity(from: SIMD3<Float>(baseX + leftX, currentY + topY, 0), to: SIMD3<Float>(baseX + leftX, currentY + bottomY, 0), thickness: lineThickness * 1.5, color: UIColor.purple.withAlphaComponent(0.4)))
                            textContainer.addChild(createLineEntity(from: SIMD3<Float>(baseX + rightX, currentY + topY, 0), to: SIMD3<Float>(baseX + rightX, currentY + bottomY, 0), thickness: lineThickness * 1.5, color: UIColor.purple.withAlphaComponent(0.4)))
                            textContainer.addChild(createLineEntity(from: SIMD3<Float>(baseX + leftX, currentY + bottomY, 0), to: SIMD3<Float>(baseX + rightX, currentY + bottomY, 0), thickness: lineThickness * 1.5, color: UIColor.purple.withAlphaComponent(0.4)))
                        }
                    }
                ...
                case .koten:
                    ...
                    let drawKoten = { (baseX: Float) in
                        for point in intersections {
                            let dotMesh = MeshResource.generateSphere(radius: 0.0008) // 始点より少し大きめの0.8ミリ
                            var dotMat = UnlitMaterial(color: UIColor.magenta.withAlphaComponent(0.6)) // マゼンタ色で差別化
        ...
        func session(_ session: ARSession, didUpdate frame: ARFrame) {
            ...
            if let result = arView.raycast(from: screenCenter, allowing: .estimatedPlane, alignment: .horizontal).first {
                ...
                let dx = cameraPos.x - anchorPos.x, dz = cameraPos.z - anchorPos.z
...
func getCharacterMetrics(char: Character, font: CTFont) -> (topWidth: Float, bottomWidth: Float, height: Float) {
    ...
    var topMinX = bounds.maxX, topMaxX = bounds.minX; var bottomMinX = bounds.maxX, bottomMaxX = bounds.minX
    path.applyWithBlock { elementPointer in
        let element = elementPointer.pointee; let points = element.points; let numPoints: Int
        switch element.type {
    ...
    return (max(Float((topMaxX - topMinX) * arScale), 0.0015), max(Float((bottomMaxX - bottomMinX) * arScale), 0.0015), max(Float(bounds.height * arScale), 0.0015))
}

func createLineEntity(from p1: SIMD3<Float>, to p2: SIMD3<Float>, thickness: Float, color: UIColor) -> ModelEntity {
    let dx = p2.x - p1.x, dy = p2.y - p1.y, length = sqrt(dx*dx + dy*dy)
    ...
    line.position = [(p1.x + p2.x) / 2, (p1.y + p2.y) / 2, 0]; line.orientation = simd_quatf(angle: atan2(dy, dx), axis: [0, 0, 1])
    return line
}
...
func createDashedLineEntity(from p1: SIMD3<Float>, to p2: SIMD3<Float>, thickness: Float, color: UIColor, dashLength: Float = 0.0008, gapLength: Float = 0.0008) -> Entity {
    ...
    let dx = p2.x - p1.x, dy = p2.y - p1.y, totalLength = sqrt(dx*dx + dy*dy)
    ...
    while currentDist < totalLength {
        ...
        line.orientation = simd_quatf(angle: atan2(dy, dx), axis: [0, 0, 1]); parentEntity.addChild(line)
...
func createRectangularFrame(width: Float, height: Float, thickness: Float, color: UIColor) -> Entity {
    let frame = Entity(), w = width / 2, h = height / 2
    let topLeft = SIMD3<Float>(-w, h, 0), topRight = SIMD3<Float>(w, h, 0), bottomLeft = SIMD3<Float>(-w, -h, 0), bottomRight = SIMD3<Float>(w, -h, 0)
